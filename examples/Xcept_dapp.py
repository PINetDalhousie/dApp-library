...
from spectrum.Xcept_dapp import XceptDApp

LOG_DIR = ('.' if os.geteuid() != 0 else '') + '/logs/'

def stop_program(time_to_wait, dapp: XceptDApp):
...
def main(args, time_to_wait: float = 500.0):
    ...
    dapp = XceptDApp(noise_floor_threshold=noise_floor_threshold, id=args.id, input_size=args.input_size, 
                     input_rate=args.input_rate, output_size=args.output_size, model_deployment=args.model_deployment, 
                     model_type=args.model_type, save_iqs=args.save_iqs, control=args.control, link=args.link, 
                     transport=args.transport, energyGui=args.energy_gui, iqPlotterGui=args.iq_plotter_gui, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input-rate', type=int,  default=63, help="Specify input rate for the dApp")
