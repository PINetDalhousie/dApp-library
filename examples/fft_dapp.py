...
from spectrum.fft_dapp import FFTDApp

LOG_DIR = ('.' if os.geteuid() != 0 else '') + '/logs/'

def stop_program(time_to_wait, dapp: FFTDApp):
...
def main(args, time_to_wait: float = 500.0):
    ...
    dapp = FFTDApp(noise_floor_threshold=noise_floor_threshold, id=args.id, input_rate=args.input_rate, input_size=args.input_size, output_size=args.output_size, save_iqs=args.save_iqs, control=args.control, link=args.link, transport=args.transport,
